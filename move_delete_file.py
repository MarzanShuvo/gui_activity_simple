...
CHECK_INTERVAL = 10*60  # Check every 10 seconds within the 30-minute run
...
def delete_files_in_container(container_id, files, container_directory):
    client = docker.from_env()
    container = client.containers.get(container_id)
    for file in files:
        if file.startswith('rosbag'):
            command = f"rm -r {os.path.join(container_directory, file)}"
            exit_code, output = container.exec_run(command)
            if exit_code != 0:
                log_and_print(f"Error deleting file {file} in container: {output.decode('utf-8')}")
            else:
                log_and_print(f"Deleted {file} from container.")
...
def move_new_rosbag_folders():
    ...
    try:
        ...
        # Move new files to host directory and delete them from the container
        if new_files:
            ...
            # Delete the moved files that start with 'rosbag' from the container
            files_to_delete = [file for file in new_files if file.startswith('rosbag')]
            delete_files_in_container(container_id, files_to_delete, CONTAINER_DIRECTORY)
        else:
...
def schedule_task():
    log_and_print("Scheduling task to run every 7 hours.")
    schedule.every(7).hours.do(run_for_30_minutes)  # Schedule the function to run every 7 hours
